  ...
  return (
    <div className="relative flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md glass-strong rounded-2xl p-8 shadow-xl"
      >
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-6 font-display text-2xl font-bold">{t('auth.welcomeBack')}</h1>
        ...
        <div className="relative my-6">
          ...
          <span className="relative mx-auto block w-fit bg-card px-2 text-xs text-muted-foreground">{t('auth.continueWith')}</span>
        </div>

        <Button variant="outline" className="w-full rounded-xl" type="button" onClick={() => toast.info(t('auth.googleMock'))}>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          {t('auth.noAccount')}{' '}
          <Link to="/register" className="font-medium text-primary hover:underline">{t('auth.createAccount')}</Link>
        </p>
      </motion.div>
    </div>
  )
