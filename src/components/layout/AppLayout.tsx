...
import {
  ...
  Search, Sparkles, PanelLeftClose, PanelLeft, Loader2,
} from 'lucide-react'
    ...
    <div className={cn('flex min-h-screen bg-background', focusMode && 'focus-mode')} data-focus-mode={focusMode || undefined}>
      ...
      >
        <div className="flex h-16 items-center gap-2 border-b px-4">
          <Link to="/app/dashboard" className="flex items-center gap-2 font-display font-bold">
            ...
            {sidebarOpen && <span>{t('common.appName')}</span>}
          </Link>
        </div>
        ...
        <div className="border-t p-3 space-y-2">
          <Button
            ...
            aria-label={t('layout.openCommand')}
          >
            <Search className="h-4 w-4" />
            {sidebarOpen && <span className="flex-1 text-left text-muted-foreground">{t('common.search')}… ⌘K</span>}
          </Button>
          <div className={cn('flex items-center gap-2', !sidebarOpen && 'justify-center')}>
            <LanguageSwitcher compact />
            <ThemeToggle compact />
            <Button variant="ghost" size="icon" onClick={toggleSidebar} aria-label={t('layout.toggleSidebar')}>
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b glass px-4 lg:px-6">
          <Button variant="ghost" size="icon" className="lg:hidden" onClick={toggleSidebar} aria-label={t('layout.openMenu')}>
          ...
          <div className="flex items-center gap-2">
            {isLoading && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" aria-label={t('data.syncing')} />}
            <Button variant="ghost" size="icon" className="relative" aria-label={t('layout.notificationsUnread', { count: unreadCount })}>
